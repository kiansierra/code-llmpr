...
import wandb
from llm_prompt import CosineScorer, LlamaFormatter
...
def parser():
    ...
    argparser.add_argument("--version", type=int, required=False, default=None)
...
def main(args) -> None:
    ...
    dataset = dataset.map(
        batch_generator(model, tokenizer, **generation_kwargs),
        batched=True,
